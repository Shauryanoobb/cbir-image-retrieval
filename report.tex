...
\begin{titlepage}
    ...
    \begin{abstract}
        ...
        Wang dataset (1000 images, 10 classes), using every image as a query in a
...
\begin{itemize}[noitemsep]
    ...
    \item A comprehensive leave-one-out evaluation over all 1000 images, reporting
...
Given a database $\mathcal{D} = \{I_1, I_2, \ldots, I_N\}$ of $N = 1000$ images and a
...
\begin{enumerate}[noitemsep]
    ...
    \item \textbf{Similarity ranking.} A distance metric (Euclidean or cosine) between the
          query vector and every database vector is computed, and images are ranked in
...
Two distance metrics are evaluated. \textbf{Euclidean distance} measures the straight-line
distance between feature vectors:

\begin{equation}
    \delta_{\text{euc}}(\mathbf{f}_q, \mathbf{f}_i)
    = \|\mathbf{f}_q - \mathbf{f}_i\|_2
    = \sqrt{\sum_{k=1}^{d} (f_{q,k} - f_{i,k})^2}
\end{equation}

\textbf{Cosine distance} measures the angle between vectors, making it invariant to
vector magnitude:

\begin{equation}
    \delta_{\text{cos}}(\mathbf{f}_q, \mathbf{f}_i)
    = 1 - \frac{\mathbf{f}_q \cdot \mathbf{f}_i}{\|\mathbf{f}_q\|_2\,\|\mathbf{f}_i\|_2}
\end{equation}

It is worth noting that for unit-norm vectors the two metrics produce identical rankings,
since $\delta_{\text{euc}}^2 = 2\,\delta_{\text{cos}}$ when
$\|\mathbf{f}_q\|_2 = \|\mathbf{f}_i\|_2 = 1$. The color histogram vectors are
$\ell^2$-normalized by construction, and the combined extractors normalize each component
before concatenation, so the metric choice only meaningfully affects the two
texture-only descriptors (LBP and GLCM) whose raw outputs are not unit-normalized.

All database images are ranked by distance in ascending order, and the query image itself
is excluded from the returned results before precision is computed.

\subsection{Evaluation Protocol}

Performance is measured using a \textbf{leave-one-out} protocol: each of the 1000 images
...
\begin{itemize}[noitemsep]
    \item \textbf{Global}: macro-average over all 1000 queries.
...
averaged over all 1000 queries. Several clear trends emerge.

\begin{table}[h]
    \centering
    \caption{Global Precision@$K$ averaged over all 1000 queries on the Wang dataset.
...
\textbf{Effect of distance metric.} Switching from Euclidean to cosine distance produces
no change whatsoever for the color histogram or any combined extractor — a direct
consequence of those vectors already being unit-normalized, making the two metrics
algebraically equivalent in ranking order. The only extractors where the choice matters are
the two texture-only descriptors: cosine distance gives LBP a small improvement
(P@10: 0.4893 $\to$ 0.5113), while Euclidean distance is notably better for GLCM
(P@10: 0.3339 vs 0.2856 under cosine). The GLCM result is expected — its raw feature
values are unnormalized statistics with very different scales across dimensions, and
Euclidean distance implicitly gives higher weight to larger-magnitude dimensions, which
happens to be beneficial here. Overall, the distance metric has negligible practical impact
on this system, and all reported results use Euclidean distance.
